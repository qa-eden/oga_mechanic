...
const ChatMechanic = () => {
  ...
  const mechanicData = {
    ...
    reviewCount: 30,
    image: (params.mechanicImage as string) || "/placeholder.svg?height=60&width=60",
    phone: "08056432765",
    isOnline: true,
  }
    ...
    <View className={`mb-4 ${item.isSent ? "items-end" : "items-start"}`}>
      <View
        className={`max-w-[80%] px-4 py-3 rounded-2xl ${
          item.isSent ? "bg-primary-500 rounded-br-md" : "bg-gray-200 rounded-bl-md"
        }`}
      >
        <Text className={`text-base leading-6 font-NunitoMedium ${item.isSent ? "text-white" : "text-gray-800"}`}>
          {item.text}
        </Text>
      </View>
      <View className={`flex-row items-center mt-1 px-2 ${item.isSent ? "justify-end" : "justify-start"}`}>
        <Text className="text-xs text-gray-500 mr-1 font-NunitoMedium">{item.timestamp}</Text>
        {item.isSent && (
          <View className="flex-row items-center">
            {item.status === "sending" && <View className="w-3 h-3 border border-gray-400 rounded-full" />}
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
      ...
      <View className="flex-row items-center justify-between px-5 py-4 bg-white border-b border-gray-100 shadow-sm">
        <BackArrowBtn />
        <Text className="text-lg font-NunitoBold text-gray-900">Chat mechanic</Text>
        <View className="w-10" />
      </View>
      ...
      <View className="flex-row items-center justify-between px-5 py-4 bg-white border-b border-gray-100">
        <View className="flex-row items-center flex-1">
          ...
          <View className="flex-1">
            <Text className="text-base font-NunitoBold text-gray-900">{mechanicData.name}</Text>
      ...
      >
        ...
        <View className="px-5 pt-4 pb-8 bg-white border-t border-gray-100">
          <View className="flex-row items-center space-x-3">
            ...
            >
              <PaperAirplaneIcon size={20} color={inputText.trim() ? "#FFFFFF" : "#9CA3AF"} />
            </TouchableOpacity>
          </View>
        </View>
      </KeyboardAvoidingView>
